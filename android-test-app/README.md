...
> Para emulador Android puedes usar `http://10.0.2.2:8080/` (opcional).

Por defecto este módulo ya queda apuntando a `http://192.168.1.4:8080/`.
...
- `network_security_config.xml`: tráfico HTTP habilitado en general para pruebas locales (cleartext).
...
## Troubleshooting
- Si aparece `Cleartext communication to 10.0.2.2 not permitted by network security`, verifica que el `AndroidManifest.xml` tenga `android:networkSecurityConfig="@xml/network_security_config"` y que el archivo use `base-config cleartextTrafficPermitted="true"`.
